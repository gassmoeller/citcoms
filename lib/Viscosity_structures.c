     ...
     int propogate;
{
	...
	// Bernhards Viscosity profile
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)
		    ...
		    if (zzz<=0.3453 && zzz>0.3233){
			EEta[m][ (i-1)*vpts + jj ] = 111;}
		    if (zzz<=0.3233 && zzz>0.3014){
			EEta[m][ (i-1)*vpts + jj ] = 101;}
		    if (zzz<=0.3014 && zzz>0.2794){
			EEta[m][ (i-1)*vpts + jj ] = 87.5;}
		    if (zzz<=0.2794 && zzz>0.2574){
			EEta[m][ (i-1)*vpts + jj ] = 73.1;}
		    if (zzz<=0.2574 && zzz>0.2354){
			EEta[m][ (i-1)*vpts + jj ] = 58.6;}
		    if (zzz<=0.2354 && zzz>0.2135){
			EEta[m][ (i-1)*vpts + jj ] = 45.1;}
		    if (zzz<=0.2135 && zzz>0.1915){
			EEta[m][ (i-1)*vpts + jj ] = 33.2;}
		    if (zzz<=0.1915 && zzz>0.1695){
			EEta[m][ (i-1)*vpts + jj ] = 23.5;}
		    if (zzz<=0.1695 && zzz>0.1475){
			EEta[m][ (i-1)*vpts + jj ] = 15.9;}
		    if (zzz<=0.1475 && zzz>0.1256){
			EEta[m][ (i-1)*vpts + jj ] = 10.3;}
		    if (zzz<=0.1256 && zzz>0.1036){
			EEta[m][ (i-1)*vpts + jj ] = 6.24;}
		    if (zzz<=0.1036 && zzz>0.0816){
			EEta[m][ (i-1)*vpts + jj ] = 0.651;}
		    if (zzz<=0.0816 && zzz>0.0644){
			EEta[m][ (i-1)*vpts + jj ] = 0.431;}
		    if (zzz<=0.0644 && zzz>0.0534){
			EEta[m][ (i-1)*vpts + jj ] = 0.432;}
		    if (zzz<=0.0534 && zzz>0.0345){
			EEta[m][ (i-1)*vpts + jj ] = 0.290;}
		    if (zzz<=0.0345 && zzz>0.0157){
		    	EEta[m][ (i-1)*vpts + jj ] = 0.241;}
		    if (zzz<= 0.0157 && zzz>=0){
			EEta[m][ (i-1)*vpts + jj ] = 264;}
		}}
		break;
		    //EEta[m][ (i-1)*vpts + jj ] = tempa*
		    //  exp( (E->viscosity.E[l] +  E->viscosity.Z[l]*zzz )
		    //	   / (E->viscosity.T[l]+temp) );
    case 101:
	// thicker lithosphere --> leads to slow convergence
        /* user-defined viscosity law goes here */
        //fprintf(stderr, "Need user definition for viscosity law: 'rheol=%d'\n",
        //        E->viscosity.RHEOL);
        //parallel_process_termination();
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)
		    ...
		    if (zzz<=0.0644 && zzz>0.0534){
			EEta[m][ (i-1)*vpts + jj ] = 0.432;}
		    if (zzz<=0.0534 && zzz>0.025){
			EEta[m][ (i-1)*vpts + jj ] = 0.290;}
		    //if (zzz<=0.0345 && zzz>=0.0157){
		    //if (zzz<=0.0345 && zzz>0.025){
			//EEta[m][ (i-1)*vpts + jj ] = 0.241;}
		    //if (zzz<=0.0157 && zzz>=0){
		    if (zzz<= 0.025 && zzz>=0){
			EEta[m][ (i-1)*vpts + jj ] = 264;}
        ...
        break;
    case 102:
	// Bernhards Viscosity profile without lithosphere
        /* user-defined viscosity law goes here */
        //fprintf(stderr, "Need user definition for viscosity law: 'rheol=%d'\n",
        //        E->viscosity.RHEOL);
        //parallel_process_termination();
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)
		    ...
		    if (zzz>=0.4332){
			//EEta[m][ (i-1)*vpts + jj ] = 0.186;}
			EEta[m][ (i-1)*vpts + jj ] = 3;}	
		    if (zzz<=0.4332 && zzz>0.4112){
			//EEta[m][ (i-1)*vpts + jj ] = 3.54;}
			EEta[m][ (i-1)*vpts + jj ] = 9;}
		    if (zzz<=0.4112 && zzz>0.3893){
			EEta[m][ (i-1)*vpts + jj ] = 33.2;}
		    if (zzz<=0.3893 && zzz>0.3673){
			EEta[m][ (i-1)*vpts + jj ] = 88.5;}
		...
		break;
    case 103:
	// Bernhards Viscosity profile with Temperature Dependency
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)
			...
			EEta[m][ (i-1)*vpts + jj ] = EEta[m][ (i-1)*vpts + jj ] * exp(-14.0*(temp-0.5));}
		    if (zzz<=0.4332 && zzz>0.4112){
			EEta[m][ (i-1)*vpts + jj ] = 3.54;
			EEta[m][ (i-1)*vpts + jj ] = EEta[m][ (i-1)*vpts + jj ] * exp(-14.0*(temp-0.5));}
		    if (zzz<=0.4112 && zzz>0.3893){
    ...
    case 104:
		...
		if (zzz>0.36){	
		    EEta[m][ (i-1)*vpts + jj ] = 0.04*
		      exp( (25 -25*zzz)
			   / (temp+1.2) );
			}
		if (zzz<= 0.36 && zzz>0.12){
		    EEta[m][ (i-1)*vpts + jj ] = 35000*
		      exp( (-18 +18*zzz)
			   / (temp+1.2) );
			}
		if (zzz<=0.12) {
		    EEta[m][ (i-1)*vpts + jj ] = 0.5*
		      exp( (5 -5*zzz)
			   / (temp+1.2) );
			}
    ...
    case 105:
	// constant Viscosity profile with Temperature Dependency
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)
		   ...
		   /* if (zzz>=0.4332){
		    ...
		    if (zzz<=0.0345 && zzz>0.0157){
		    	EEta[m][ (i-1)*vpts + jj ] = 0.241;}
		    if (zzz<= 0.0157 && zzz>=0){
			EEta[m][ (i-1)*vpts + jj ] = 2.64;}*/
		  	tempa = E->viscosity.N0[l];
			EEta[m][ (i-1)*vpts + jj ] = tempa * exp(-14.0*(temp-0.5));
		}}
		break;
		    //EEta[m][ (i-1)*vpts + jj ] = tempa*
		    //  exp( (E->viscosity.E[l] +  E->viscosity.Z[l]*zzz )
